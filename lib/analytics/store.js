...
// Records a poll attempt without a snapshot. Without this, a post whose poll
// failed stayed "due" and was retried on every 5-minute collector cycle —
// which is what drove the app into Meta's app-level rate limit.
export async function markPolled(postMetricId) {
  await query('UPDATE post_metrics SET last_polled_at = now() WHERE id = $1', [postMetricId])
}
...
export async function getTimeseriesSince(userId, sinceISO) {
  const { rows } = await query(
    `SELECT ms.captured_at, ms.likes, ms.comments, ms.shares, ms.views, ms.post_metric_id, pm.platform, p.id AS post_id
