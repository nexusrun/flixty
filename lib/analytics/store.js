...
export async function getPostsNeedingDiscovery() {
  const { rows } = await query(
    `SELECT id, platforms, results, published_at FROM posts
...
export async function getDuePostMetrics() {
  const { rows } = await query(`
    SELECT pm.*, p.platforms, p.published_at
...
export async function getLatestSnapshotsSince(sinceISO) {
  const { rows } = await query(
...
export async function getTimeseriesSince(sinceISO) {
  const { rows } = await query(
...
export async function getLatestInsight() {
  const { rows } = await query('SELECT * FROM analytics_insights ORDER BY generated_at DESC LIMIT 1')
  return rows[0] || null
}

export async function saveInsight({ periodStart, periodEnd, summary, raw = {} }) {
  const { rows } = await query(
    `INSERT INTO analytics_insights (period_start, period_end, summary, raw)
     VALUES ($1, $2, $3, $4) RETURNING *`,
    [periodStart, periodEnd, summary, raw]
  )
  return rows[0]
}
