...
import {
  ...
  getDuePostMetrics, recordSnapshot, markPolled,
} from './store.js'

// Facebook and Instagram share one app-wide Graph API quota (Facebook Login
// needs it too). When Meta says we're throttled, stop polling both for a
// while so the quota can recover instead of burning it on retries.
const META_PLATFORMS = new Set(['facebook', 'instagram'])
const META_BACKOFF_MS = 60 * 60 * 1000
let metaPausedUntil = 0
...
async function pollDueMetrics(getTokensForUser) {
  ...
  for (const pm of due) {
    if (META_PLATFORMS.has(pm.platform) && Date.now() < metaPausedUntil) continue
    try {
      ...
      if (META_PLATFORMS.has(pm.platform) && facebook.isRateLimitError(e)) {
        metaPausedUntil = Date.now() + META_BACKOFF_MS
        console.warn(`[analytics] Meta rate limit hit — pausing Facebook/Instagram polling until ${new Date(metaPausedUntil).toISOString()}`)
        continue // not this post's fault; leave it due for when polling resumes
      }
      // Wait for the normal cadence before retrying rather than every cycle.
      await markPolled(pm.id).catch(err => console.warn(`[analytics] could not mark post_metric ${pm.id} polled:`, err.message))
    }
  }
}
