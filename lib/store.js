...
export async function updateScheduled(userId, id, post) {
  const { text, platforms = [], accountTargets = {}, scheduledAt, imageUrl = null, campaignName = null } = post
  const { rows } = await query(
    `UPDATE scheduled_posts SET text = $1, platforms = $2, account_targets = $3, scheduled_at = $4, image_url = $5, campaign_name = $6
     WHERE id = $7 AND user_id = $8
     RETURNING *`,
    [text, platforms, accountTargets, scheduledAt, imageUrl, campaignName, id, userId]
  )
